...
bg_surface = pygame.image.load('images/background-day.png').convert() # load the backgound image to a variable  .... .convert() will convert the image to the type that is easier for python to work with
bg_surface = pygame.transform.scale2x(bg_surface) # scaling the size of the background image by 2x 
...
while True:
	...
	screen.blit(bg_surface,(0,0)) # this will show on the half of the screen but since we have scaled the bg_surface by 2x it will displayed on the full screen
	...
	floor_x_position -=1
	draw_floor()
	if floor_x_position<= -576: # when the positon of the floor is farthest to the left the if condition will become true and the floor position will be changed to 0 
		floor_x_position = 0 
	

	pygame.display.update()
	clock.tick(120) # this will control the highest frame rate which at this point can be 120 
